...
module.exports = class extends think.Service {
  async login(code, fullUserInfo) {
    let myurl='https://api.weixin.qq.com/sns/jscode2session?appid='+think.config('weixin.appid')+'&&secret='+think.config('weixin.secret')+'&&js_code='+code+'&&grant_type=authorization_code&connect_redirect=1';
    //console.log('url:',myurl);
    try {
      // 获取 session
      //console.log('---options---',code);
      //console.log('---options---',think.config('weixin.secret'));
      //console.log('---options---',think.config('weixin.appid'));
      const options = {
        method: 'GET',
        url: myurl,
        //qs: {
      ...
      //console.log('get in session Data');
      let sessionData = await rp(options);
      //console.log('sessionDate is ',sessionData);
      ...
      if (fullUserInfo.signature !== sha1) {
        console.log('fullUserInfo is not full!',sha1);
        return null;
      }
